...
In the past decade research has indicated that it is optimal for babies to be exclusively breast fed until 6 months-old (Kramer & Kakuma, 2012). However, not all mothers breast feed their babies, and from those who do it, not all of them continue until the sixth month. The reasons to stop breast feeding can be very diverse: they can be physical, psychological or even cultural (Wright & Schanler, 2001).
...
The response variable is the duration of breast feeding. Thus, the time origin is the birth of the child and the event of interest is stopping breast feeding, signaled by the indicator variable ('delta'). 
Table 1 shows the variables included in the database.


```{r echo=FALSE, error=FALSE, message=FALSE, warning=FALSE}
a<-c("duration","delta","race","poverty","smoke","alcohol","agemth","ybirth","yschool","pc3mth")
b<-c("Duration of breast feeding in weeks","Indicator of completed breast feeding (1=yes, 0=no)","Race of mother (1=white, 2=black, 3=other)","Mother in poverty (1=yes, 0=no)","Mother smoked at birth of child (1=yes, 0=no)","Mother used alcohol at birth of child (1=yes, 0=no)","Age of mother at birth of child in years","Year of birth","Education level of mother (years of school)","Prenatal care after third month (1=yes, 0=no)")
c<-cbind(a,b)
colnames(c)<-c("Variable","Description")
knitr::kable(c,caption= "Description of variables in bfeed", row.names=FALSE, align=c("l", "l", "r", "r"))
```

Note that agemth, ybirth and yschool are continuous variables while race, poverty, smoke, alcohol,and pc3mth are categorical variables (the former with 3 levels are the others with 2 levels). 
The 'bfeed' database has 927 observations and no NA values. Only 35 observations were censored.

Tables 2 and 3 show statistics for the categorial and continuous variables, respectively.
...
bfeed %>%
  summary_factorlist(explanatory=explanatory, add_dependent_label=TRUE) -> t1
#t1



bfeed%>%group_by(race,poverty,smoke, alcohol,pc3mth,delta) %>%
        summarize(N = n()) %>%
        group_by(race,poverty,smoke, alcohol,pc3mth) %>% mutate(Tot = sum(N)) %>%
        melt(measure.vars  = c("race","poverty","smoke","alcohol","pc3mth")) %>% group_by(variable,value) %>% mutate(GT = sum(N)) %>%
        subset(delta == 0) %>%
        summarize(c1 = sum(N),Tot = sum(Tot),GT =GT[1], C = sum(N)/GT[1])  -> t2

t1$`censored (%)` <- sprintf("%d (%.1f)", t2$c1, t2$C*100)

names(t1) <- c("Variable","X","N(%)","Censored(%)")

t1$X <- c("White","Black","Other","No","Yes","No","Yes","No","Yes","No","Yes")

names(t1) <- c("Variable","","N(%)","Censored(%)")

knitr::kable(t1, caption= "Statistics of the categorical variables", row.names=FALSE, align=c("l", "l", "r", "r"))

```

```{r, echo=FALSE, warning=FALSE,error=FALSE,message=FALSE}

aux <- bfeed %>% select(agemth,ybirth,yschool) %>% melt() %>% group_by(variable) %>% summarise(Mean = mean(value),SD = sd(value), Min = min(value), Max = max(value))

knitr::kable(aux, caption= "Statistics of the continuous variables", row.names=FALSE, align=c("l", "l", "r", "r"),digits=3)
...
fit1<-survfit(Surv(duration,delta)~1,data=bfeed) #KM
fit2<-survfit(Surv(duration,rep(1,length(duration)))~1,data=bfeed) #empirico

plot(fit1,xlab="time",ylab="Survival",conf.int = FALSE,main="")
lines(fit2,xlab="time",ylab="Survival",conf.int = FALSE,add=TRUE,col=2)
legend("topright",legend=c("KM","Empirical"),text.col=1:2)

```

The median survival time, i.e., the time at which 50% of the events have occurred can be estimated from the curve, and it is 12 weeks. The mean residual time is 16.87 weeks (SE = 0.614). Table 4 shows the survival estimates at different moments in time.
...
```{r, echo=FALSE, warning=FALSE,error=FALSE,message=FALSE}
km <- survfit(Surv(duration,delta)~1,data=bfeed)
x <- summary(km, times=c(1,4,8,12,16,20,24,32,40,48,72,96,144,192))
tableSurv<-cbind(x$time,round(x$surv,3),
                 c("0.899-0.935",
                   "0.682-0.741",
                   "0.500-0.565",
                   "0.388-0.453",
...
From Table 4 we can see that after aproximately 1 month (4 weeks) only 71% of mothers were still breast feeding. We can also use the Kaplan-Meier estimator to estimate survival in different groups. At the end of approximately 6 months (24 weeks), the recommended time to feed babies with only milk (preferably, the mother's milk) the percentage of mothers still breast feeding drops to about 22%. We can also see that even though they are rare, there are mothers breast feeding their babies for more than 3 years.
...
# plot(km.race,main=" Kaplan-Meier estimates for race", xlab="weeks", ylab="survival",
#      col=1:3, lty=1:3)
...
# plot(km.poverty,main=" Kaplan-Meier estimate for poverty", xlab="weeks", ylab="survival",
#      col=1:2, lty=1:2)
...
# Thus, survival does not differ across poverty.


km.smoke<-survfit(Surv(duration,delta)~smoke,data=bfeed)
# plot(km.smoke,main=" Kaplan-Meier estimate for smoke", xlab="weeks", ylab="survival",
#      col=1:2, lty=1:2)
# legend("topright",c("yes","no"),col=1:2,lty=1:2)
...
# plot(km.alcohol,main=" Kaplan-Meier estimate for alcohol", xlab="weeks", ylab="survival",
#      col=1:2, lty=1:2)
# legend("topright",c("yes","no"),col=1:2,lty=1:2)
LR.alcohol<-survdiff(Surv(duration,delta)~alcohol,data=bfeed)
# With the log-rank test, with a significance level of 5% we do not reject H0.
# Thus, survival does not differ across alcohol

km.pc3mth<-survfit(Surv(duration,delta)~pc3mth,data=bfeed)
# plot(km.pc3mth,main=" Kaplan-Meier estimate for pc3mth", xlab="weeks", ylab="survival",
#      col=1:2, lty=1:2)
# legend("topright",c("yes","no"),col=1:2,lty=1:2)
LR.pc3mth<-survdiff(Surv(duration,delta)~pc3mth,data=bfeed)
# With the log-rank test, with a significance level of 5% we do not reject H0.
# Thus, survival does not differ across pc3mth
...
To check if there were differences in the survival curves according to the mother's age, mother's education or baby's birth year, these variables were categorized such that each category holds approximately the same number of observations (or at least a symmetrical number). The resulting distributions are presented in Tables 6, 7 and 8.



```{r, echo=FALSE, warning=FALSE,error=FALSE,message=FALSE}

age1 <- cut(bfeed$agemth, breaks=c(0, 18,21,24,28)) 
#table(age1)
bfeed<-cbind(bfeed,age1)
km.age1<-survfit(Surv(duration,delta)~age1,data=bfeed)
# plot(km.age1,main=" Kaplan-Meier estimate for age", xlab="weeks", ylab="survival", col=1:4, lty=1:4)
# legend("topright",c("15-18","18-21","21-24","24-28"),col=1:4,lty=1:4)
LR.age1<-survdiff(Surv(duration,delta)~age1,data=bfeed)
#no diff

a<-c("15-18","18-21","21-24","24-28")
b<-c(110,370,303,144)
c<-cbind(a,b)
colnames(c)<-c("Mother Age categories","N")

knitr::kable(c, caption= "Mother Age categories", row.names=FALSE, align=c("l", "l", "r", "l"))
```
...
```{r, echo=FALSE, warning=FALSE,error=FALSE,message=FALSE}
#table(bfeed$ybirth)
y2 <- cut(bfeed$ybirth, breaks=c(0, 80,83,86)) #breaks= aberto à esquerda, fechado à direita
#table(y2)
bfeed<-cbind(bfeed,y2)
km.y2<-survfit(Surv(duration,delta)~y2,data=bfeed)
# plot(km.y2,main=" Kaplan-Meier estimate for year of birth", xlab="weeks", ylab="survival",
#      col=1:3, lty=1:3)
# legend("topright",c("78-80","81-83","84-86"),col=1:3,lty=1:3)
LR.y2<-survdiff(Surv(duration,delta)~y2,data=bfeed)
#diff
res<-pairwise_survdiff(Surv(duration,delta)~y2,data=bfeed,p.adjust.method = "BH",rho=0)

a<-c("78-80","81-83","84-86")
b<-c(249,414,264)
c<-cbind(a,b)
colnames(c)<-c("Year of birth categories","N")

knitr::kable(c, caption= "Year of birth categories", row.names=FALSE, align=c("l", "l", "r", "l"))

```

```{r, echo=FALSE, warning=FALSE,error=FALSE,message=FALSE}
#table(bfeed$yschool)
ys2 <- cut(bfeed$yschool, breaks=c(2,11, 12,19)) #breaks= aberto à esquerda, fechado à direita
#table(ys2 )
bfeed<-cbind(bfeed,ys2 )
km.ys2<-survfit(Surv(duration,delta)~ys2 ,data=bfeed)
# plot(km.ys2 ,main=" Kaplan-Meier estimate for years of school", xlab="weeks", ylab="survival",
#      col=1:3, lty=1:3)
# legend("topright",c("3-11","12","12-19"),col=1:3,lty=1:3)
LR.ys2<-survdiff(Surv(duration,delta)~ys2,data=bfeed)
#diff
res<-pairwise_survdiff(Surv(duration,delta)~ys2,data=bfeed,p.adjust.method = "BH",rho=0)

a<-c("3-11","12","12-19")
b<-c(220,438,269)
c<-cbind(a,b)
colnames(c)<-c("Years of school categories","N")

knitr::kable(c, caption= "Years of school categories", row.names=FALSE, align=c("l", "l", "r", "l"))
```
...
```{r, echo=FALSE, warning=FALSE,error=FALSE,message=FALSE}
LR<-rbind(
  cbind("agemth",round(LR.age1$chisq,3),2,round(1-pchisq(LR.age1$chisq,2),3)),
  cbind("ybirth",round(LR.y2$chisq,3),1,round(1-pchisq(LR.y2$chisq,1),3)),
  cbind("yschool",round(LR.ys2$chisq,3),1,round(1-pchisq(LR.ys2$chisq,1),3))
  )
...
# summary(cox.ybirth)
# #year of birth significantly affects the hazard ratio. Per year, the risk increases 5%
...
# summary(cox.yschool)
# #years of school significantly affects the hazard ratio. Per year, the risk decreases 5%
...
# round(exp(cox.pc3mth$coefficients[1]+qnorm(0.975)*sqrt(cox.pc3mth$var[1,1])),3)#IC
# round(2*(1-pnorm(0.399)),3)



t2<-c("Variable", " ", "HR","95% CI", "p-value")
ucox<-rbind(
...
knitr::kable(ucox, caption= "Univariate Cox regression.", row.names=FALSE, align=c("l", "l", "l", "l","l"))
```
